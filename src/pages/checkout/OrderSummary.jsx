      ...
      >
        ...
        {cartDetails.items.map((item, index) => {
          ...
          const enProductSize = enSizes.find(
            (size) => size?.id?.toString() === item?.size?.toString()
          );
          const arProductSize = arSizes.find(
            (size) => size?.id?.toString() === item?.ar_size?.toString()
          );
        ...
        >
          ...
          <Typography variant="body2" color="textDisabled">
            {isRTL
              ? cartDetails.totalPrice.toFixed(3) + " ر۔ع"
              : "OMR " + cartDetails.totalPrice.toFixed(3)}
          </Typography>
        </Box>
        <Divider sx={{ margin: "16px 0" }} />
        <Box
        ...
        >
          <Typography variant="body2" color="textDisabled">
            ...
            {isRTL
              ? "لا يشمل رسوم التوصيل. سيتواصل معك فريقنا عبر واتساب."
              : "Excluded Delivery Charges. Our team will contact you on WhatsApp."}
          </Typography>
        </Box>
        ...
        <Box sx={{ marginTop: 3 }}>
          ...
          >
            {placeOrderLoading ? (
              <CircularProgress size={25} color={colorPalette.white} />
            ) : (
