...
export const ProductView = () => {
  ...
  const enProductBrand = enBrands.find(
    (size) => size?.id?.toString() === product?.brand?.toString()
  );
  const arProductBrand = enBrands.find(
    (size) => size?.id?.toString() === product?.ar_brand?.toString()
  );

  const enProductUnit = enUnits.find(
    (unit) => unit?.id?.toString() === product?.unit?.toString()
  );
  const arProductUnit = enUnits.find(
    (unit) => unit?.id?.toString() === product?.ar_unit?.toString()
  );

  const enProductMaterial = enMaterials.find(
    (mat) => mat?.id?.toString() === product?.material?.toString()
  );
  const arProductMaterial = enMaterials.find(
    (mat) => mat?.id?.toString() === product?.ar_material?.toString()
  );

  const enProductColor = enAvailableColors.find(
    (color) => color?.id?.toString() === product?.avalable_color?.toString()
  );
  const arProductColor = enAvailableColors.find(
    (color) => color?.id?.toString() === product?.ar_color?.toString()
  );

  const size = isRTL
    ? enSizes.find(
        (size) => size?.id?.toString() === arProductSize?.id?.toString()
      )
    : enSizes.find(
        (size) => size?.id?.toString() === enProductSize?.id?.toString()
      );
  const brand = isRTL
    ? enBrands.find(
        (brand) => brand?.id?.toString() === arProductBrand?.id?.toString()
      )
    : enBrands.find(
        (brand) => brand?.id?.toString() === enProductBrand?.id?.toString()
      );
  const unit = isRTL
    ? enUnits.find(
        (unit) => unit?.id?.toString() === arProductUnit?.id?.toString()
      )
    : enUnits.find(
        (unit) => unit?.id?.toString() === enProductUnit?.id?.toString()
      );
  const material = isRTL
    ? enMaterials.find(
        (material) =>
          material?.id?.toString() === arProductMaterial?.id?.toString()
      )
    : enMaterials.find(
        (material) =>
          material?.id?.toString() === enProductMaterial?.id?.toString()
      );
  const color = isRTL
    ? enAvailableColors.find(
        (color) => color?.id?.toString() === arProductColor?.id?.toString()
      )
    : enAvailableColors.find(
        (color) => color?.id?.toString() === enProductColor?.id?.toString()
      );
  ...
  const checkWishlistProduct = async () => {
    const isWishlistProduct = await Api.checkWishlistProduct(
      product?.id,
      user?.id
    );
  ...
  const addRemoveInWishlist = () => {
    ...
    dispatch(
      addRemoveToWishlist({
        ...
        userId: user.id,
        type: wishListItem ? "remove" : "add",
      })
    );
  };
  const productLink = `${process.env.VITE_APP_FE_URL}/product/${product?.barcode}`;
  // Drawer handlers
    ...
    <Box sx={{ padding: 3, marginTop: 15 }}>
      ...
      <Grid container spacing={2}>
        ...
        <Grid size={{ xs: 12, md: 6 }}>
          ...
          <Grid container spacing={2} mt={2}>
            {[
              {
                label: isRTL ? "مقاس: " : "Size: ",
                value: isRTL ? size?.ar_title : size?.title,
              },
              {
                label: isRTL ? "ماركة: " : "Brand: ",
                value: isRTL ? brand?.ar_title : brand?.title,
              },
              {
                label: isRTL ? "وحدة: " : "Unit: ",
                value: isRTL ? unit?.ar_title : unit?.title,
              },
              {
                label: isRTL ? "لون: " : "Color: ",
                value: isRTL ? color?.ar_title : color?.title,
              },
              {
                label: isRTL ? "رمز العنصر: " : "Material: ",
                value: isRTL ? material?.ar_title : material?.title,
              },
              {
                label: isRTL ? "الباركود: " : "Item Code: ",
      ...
      <Typography
        ...
        dangerouslySetInnerHTML={{
          __html:
            (isRTL ? product?.arabicDescription : product?.webDescription) + "",
        }}
        sx={{
          mb: 2,
          direction: isRTL ? "rtl" : "ltr",
