...
const OrderList = () => {
  ...
  const rows = {
    ...
    items:
      currentOrder?.cart?.items?.length > 0 ? currentOrder?.cart.items : [],
  };
      ...
      const baseCells = {
        ...
        price: (
          <Typography color="black">
            {isRTL ? "ر۔ع" : "OMR"} {finalPrice} x {item.quantity} ={" "}
            {item.totalPrice}
          </Typography>
        ),
      };
  ...
  useEffect(() => {
    if (orders?.length > 0 && (latestOrder || orderId)) {
      setCurrentOrder(
        orders.find(
          (item) => item.orderId === (orderId ? orderId : latestOrder)
        )
      );
  ...
  useEffect(() => {
    if (orderId && orders?.length > 0) {
      dispatch(
        toggleToast(
          true,
          `Thanks for shopping with HM. Order number: ${orderId} You are the next one!`,
          "success"
        )
      );
    }
  ...
  const infoCards = useMemo(
    () => [
      ...
      {
        label: t.total,
        value: `${isRTL ? "ر۔ع" : "OMR"} ${currentOrder?.cart?.totalPrice?.toFixed(3) || 0}`,
      },
  ...
  const details = useMemo(
    () => [
      ...
      {
        label: t.totalItems,
        value: `${currentOrder?.cart?.items?.length || 0}`,
      },
      // { label: t.deliveryTime, value: t.expressDelivery },
      {
        label: t.shippingAddress,
        value: currentOrder?.cart?.orderDetails?.shipping_address || "N/A",
      },
      {
        label: t.totalAmount,
        value: currentOrder?.cart?.totalPrice?.toFixed(3) || 0,
      },
    ],
  ...
  const InputOrderSelectField = useCallback(() => {
    return (
      <Box sx={{ marginTop: 7 }}>
        <Typography variant={isMobile ? "body2" : "body1"} fontWeight={510}>
          {isRTL ? "طلبات:" : "Orders:"}
        </Typography>
        <Select
          ...
          onChange={(e) =>
            setCurrentOrder(
              orders.find((item) => item.orderId === e.target.value)
            )
          }
          label={currentOrder}
          input={<OutlinedInput />}
    ...
    >
      ...
      >
        {/* <Button
          ...
          startIcon={
            !isRTL && <HomeIcon sx={{ marginRight: -1, marginLeft: 1 }} />
          }
          endIcon={
            isRTL && <HomeIcon sx={{ marginLeft: -1, marginRight: 1 }} />
          }
        >
          <Typography
          ...
          >
            ...
            >
              ...
              <Chip
                label={
                  isRTL
                    ? rtlStatusMapper[currentOrder?.status]
                    : currentOrder?.status
                }
                sx={{
            ...
            >
              ...
              <Chip
                label={
                  isRTL
                    ? rtlPaymentStatusMapper[currentOrder?.paymentStatus]
